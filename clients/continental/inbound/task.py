...
Follows the same two-part structure as clients/emvia/inbound/task.py:
  1. ContinentalInboundTask(BaseTask)  — extraction, validation, Excel output
  2. Flask Blueprint                    — index / process / download routes

Only two source documents (MBL + Packing List — no Invoice). Reference is a
REQUIRED manual UI field (like ETA Date) rather than an extracted/optional
override: GS Caltex's paperwork carries no PO/reference number anywhere, so
there is nothing to extract or fall back to.
"""
...
COLUMN_CONFIG = [
    ...
    {"header": "Mbl No",        "field_key": "mbl_no",         "width": 20},
    ...
    {"header": "Bags",          "field_key": "bags_count",     "width": 10, "num_format": "#,##0"},
    {"header": "Net Weight",    "field_key": "net_weight",     "width": 16, "num_format": "#,##0"},
    {"header": "Gross Weight",  "field_key": "gross_weight",   "width": 16, "num_format": "#,##0"},
    {"header": "ETA/Date",      "field_key": "eta_date",       "width": 20},
]

OUTPUT_FILENAME = "Continental_Inbound_Outcome.xlsx"
...
class ContinentalInboundTask(BaseTask):
    client_slug = "continental"
    task_slug = "inbound"
    label = "Continental Inbound"
    ...
    def process(self, files: dict, output_path: str | None = None, reference: str = "",
        ...
        # reference/eta_date are UI-entered (not extracted from the
        # documents) and apply uniformly to every row in this shipment.
        rows = build_rows(mbl_data, pkl_data, reference, eta_date)

        # ── Summary stats ───────────────────────────────────────────
        containers = set(r["container_no"] for r in rows)
        total_bags = sum(r["bags_count"] for r in rows)
...
_task = ContinentalInboundTask()

bp = Blueprint(
    "continental_inbound",
    __name__,
    template_folder="templates",
    url_prefix="/app/continental/inbound",
)


@bp.route("/")
def index():
    return render_template("continental_inbound/index.html")
