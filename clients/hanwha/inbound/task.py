"""
Hanwha Inbound — task module.

Follows the same two-part structure as clients/continental/inbound/task.py:
  1. HanwhaInboundTask(BaseTask)  — extraction, validation, Excel output
  2. Flask Blueprint               — index / process / download routes

Only two source documents (MBL + Container Report — no Invoice). Reference,
Ship Name, and ETA Date are all REQUIRED manual UI fields (like Swiss
Inbound's) — neither document carries a usable shipment reference to fall
back to, and Ship Name likewise isn't printed anywhere machine-reliable on
either document.

Unlike Swiss Inbound, there is no optional Inbound-advice batch upload and
no Product Reference column — this client has no such per-product reference
document.
"""
...
from .extractor import build_rows, carrier_display, extract_container_report, extract_mbl, validate
...
OUTPUT_FILENAME = "Hanwha_Inbound_Outcome.xlsx"
...
# every other client's Inbound task).
...
class HanwhaInboundTask(BaseTask):
    client_slug = "hanwha"
    task_slug = "inbound"
    label = "Hanwha Inbound"

    required_documents = [
        {"key": "mbl",              "label": "MBL (Bill of Lading)", "accept": ".pdf", "multiple": False},
        {"key": "container_report", "label": "Container Report",     "accept": ".pdf", "multiple": False},
    ]
    ...
    def process(self, files: dict, output_path: str | None = None, reference: str = "",
        ...
        report_data = extract_container_report(files["container_report"])

        # ── Step 2: Cross-document validation ───────────────────────
        validation = validate(mbl_data, report_data)
        ...
        # already-identified carrier — not a UI field.
        rows = build_rows(mbl_data, report_data, reference, eta_date, ship_name)
...
_task = HanwhaInboundTask()

bp = Blueprint(
    "hanwha_inbound",
    __name__,
    template_folder="templates",
    url_prefix="/app/hanwha/inbound",
)


@bp.route("/")
def index():
    return render_template("hanwha_inbound/index.html")
